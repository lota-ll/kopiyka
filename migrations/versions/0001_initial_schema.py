...
Кожна DDL-команда виконується окремим ``op.execute()``.

Це не стилістичний вибір: ``asyncpg`` (наш драйвер) виконує кожен
``execute()`` через extended query protocol PostgreSQL, який приймає
рівно ОДНУ SQL-команду в одному prepared statement. Один великий
``op.execute()`` з кількома ``CREATE TABLE`` через ``;`` падає з
``cannot insert multiple commands into a prepared statement`` —
з синхронним ``psycopg2`` (simple query protocol) це працювало б, з
``asyncpg`` — ні. ``DO $$ ... $$`` та тіла функцій лишаються одним
statement навіть із internal ``;``, бо PostgreSQL розглядає
dollar-quoted блок як єдиний строковий літерал.
"""
...
def downgrade() -> None:
    # DROP TABLE з переліком через кому — це одна команда, не кілька.
    op.execute(
